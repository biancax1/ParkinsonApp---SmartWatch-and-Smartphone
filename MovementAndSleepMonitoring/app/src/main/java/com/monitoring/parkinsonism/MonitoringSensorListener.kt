...
class MonitoringSensorListener(private val context: Context) : SensorEventListener {
    ...
    private fun handleAccelerometerData(event: SensorEvent) {
        ...
        val accelerationMagnitude = Math.sqrt((difX * difX + difY * difY + difZ * difZ).toDouble())

        // Hranica pre detekciu pohybu (môžeš experimentovať)
        val motionThreshold = 2
    ...
    private fun detectTremorWithFFT(event: SensorEvent) {
        ...
        if (tremorBufferX.size >= 256) { // Potrebujeme aspoň 1024 vzoriek
            ...
            Toast.makeText(context, "Total Tremor Today: $tremorIsOccuring", Toast.LENGTH_SHORT).show()
            sendATremorMessage()
        }
    }
